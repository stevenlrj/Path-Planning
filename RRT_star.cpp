...
class pathPlanner {
  ...
  private:
    double x_range;
    double y_range;
    double expand_length; // Length of tree expansion in every step
    int iteration;        // Maximum iterations to search for path
    double robot_size;    // Size of our robot
    RRTnode start;        // Robot start pose
    RRTnode goal;         // Robot goal pose
    vector<RRTnode> tree;
    unordered_map<int, pair<double, bool>> store;  // Store distance calculation and collision checking
  public:
...
int main() {
    ...
    // Run optimal planner
    pathPlanner op;
    vector<pair<double, double>> op_path = op.RRT_star(x_range, y_range, robot_pose, goal_pose, expand_length, iteration, obs_list, robot_size);
    if (!op_path.empty()) 
    ...
    pathPlanner sp;
    vector<pair<double, double>> sub_path = sp.RRT(x_range, y_range, robot_pose, goal_pose, expand_length, iteration, obs_list, robot_size);
    if (!sub_path.empty()) 
