...
class Search {
  ...
  private:
    ...
    int robot_size;
  public:
    vector<pair<int, int>> A_star (int xr, int yr, pair<int, int> & robot_pose, pair<int, int> & goal_pose, vector<vector<int>> & obs_list, int rs) { 
...
int main() {
    ...
    // Run optimal planner
    Search search;
    vector<pair<int, int>> path = search.A_star(x_range, y_range, robot_pose, goal_pose, obs_list, robot_size);
    if (!path.empty()) 
